...
func newFakeResponseHeader() *HeaderWriter{
	header := buf.New()
	common.Must2(header.WriteString("HTTP/1.1 200 OK"))
	common.Must2(header.WriteString(CRLF))

	common.Must2(header.WriteString("Date: " + time.Now().Format(http.TimeFormat)))
	common.Must2(header.WriteString(CRLF))

	common.Must2(header.WriteString("Server: nginx"))
	common.Must2(header.WriteString(CRLF))

	common.Must2(header.WriteString("Content-Type: text/html"))
	common.Must2(header.WriteString(CRLF))

	common.Must2(header.WriteString("Content-Length: " + strconv.Itoa(len(FakeNormalResponseHtml))))
	common.Must2(header.WriteString(CRLF))
	common.Must2(header.WriteString(CRLF))
	common.Must2(header.WriteString(FakeNormalResponseHtml))
